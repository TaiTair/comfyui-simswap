...
from .simswap_logger import logger
from simswap_utils import move_path, get_image_md5hash
import folder_paths
...
from .simswap.simswap_util.norm import SpecificNorm
...
insightface_path_old = os.path.join(models_path_old, "simswap")
insightface_models_path_old = os.path.join(insightface_path_old, "models")

models_path = folder_paths.models_dir
insightface_path = os.path.join(models_path, "simswap")
                    ...
                    if source_face is not None and src_wrong_gender == 0:
                        ...
                        if target_face is not None and wrong_gender == 0:
                            ...
                            if crop_size == 512:
                                ...
                                mode = 'None'
                            logoclass = watermark_image(path.dirname(path.abspath(__file__))+'/simswaplogo/simswaplogo.png')
                            model = create_model(opt)
                            model.eval()

                            spNorm =SpecificNorm()
                            app = Face_detect_crop(name='antelope', root=insightface_path)
                    ...
                    if source_face is not None and src_wrong_gender == 0:
                        for i, (target_img, target_face) in enumerate(zip(target_imgs, target_faces)):
                            ...
                            if target_face_single is not None and wrong_gender == 0:
                                ...
                                # print(repr(swap_result))
                                # result = face_swapper.get(swap_result, target_face_single, source_face)
