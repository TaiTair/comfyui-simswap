...
import scripts.simswap_version
from scripts.simswap_faceswap import (
    FaceSwapScript,
    get_models,
...
from scripts.simswap_logger import logger
from simswap_utils import (
    batch_tensor_to_pil,
...
SIMSWAP_MODELS_PATH = os.path.join(models_dir, "simswap")
FACE_MODELS_PATH = os.path.join(SIMSWAP_MODELS_PATH, "faces")

if not os.path.exists(SIMSWAP_MODELS_PATH):
    os.makedirs(SIMSWAP_MODELS_PATH)
...
def get_restorers():
    ...
    if len(models) == 0:
        fr_urls = [
            "https://huggingface.co/datasets/Gourieff/Simswap/resolve/main/models/facerestore_models/GFPGANv1.3.pth",
            "https://huggingface.co/datasets/Gourieff/Simswap/resolve/main/models/facerestore_models/GFPGANv1.4.pth",
            "https://huggingface.co/datasets/Gourieff/Simswap/resolve/main/models/facerestore_models/codeformer-v0.1.0.pth"
        ]
...
class Simswap:
    ...
    def execute(self, enabled, input_image, swap_model, detect_gender_source, detect_gender_input, source_faces_index, input_faces_index, console_log_level, face_restore_model, face_restore_visibility, codeformer_weight, facedetection, source_image=None, face_model=None, faces_order=None):
        ...
        result = Simswap.restore_face(self,result,face_restore_model,face_restore_visibility,codeformer_weight,facedetection)

        return (result,face_model_to_provide)


class SimswapPlusOpt:
    @classmethod
    def INPUT_TYPES(s):
        return {
            "required": {
...
class RestoreFace:
    ...
    def execute(self, image, model, visibility, codeformer_weight, facedetection):
        result = Simswap.restore_face(self,image,model,visibility,codeformer_weight,facedetection)
        return (result,)
...
NODE_CLASS_MAPPINGS = {
    "Simswap": Simswap,
    "SimswapFaceSwapOpt": SimswapPlusOpt,
    "SimswapLoadFaceModel": LoadFaceModel,
    "SimswapSaveFaceModel": SaveFaceModel,
    "SimswapRestoreFace": RestoreFace,
    "SimswapBuildFaceModel": BuildFaceModel,
    "SimswapMaskHelper": MaskHelper,
    "SimswapImageDublicator": ImageDublicator,
    "SimswapOptions": SimswapOptions,
}

NODE_DISPLAY_NAME_MAPPINGS = {
    "Simswap": "Simswap - Fast Face Swap",
    "SimswapLoadFaceModel": "Load Face Model",
    "SimswapSaveFaceModel": "Save Face Model",
    "SimswapRestoreFace": "Restore Face",
    "SimswapBuildFaceModel": "Build Blended Face Model",
    "SimswapMaskHelper": "Simswap Masking Helper",
    "SimswapImageDublicator": "Simswap Image Dublicator (List)",
    "SimswapOptions": "Simswap Options",
    "SimswapFaceSwapOpt": "Simswap - Fast Face Swap [OPTIONS]"
}
